...
export default function StudioSalesPage() {
    ...
    return (
        <div className="space-y-12 animate-in fade-in duration-700">
            ...
            <div className="bg-[#020205] border border-white/5 rounded-[3rem] p-10 relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-500 to-blue-600 opacity-20" />

                {/* Search & Filters */}
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
                    <h3 className="text-2xl font-black uppercase tracking-tighter shrink-0">
                        Libro de <span className="text-accent underline underline-offset-4 decoration-white/10">Transacciones</span>
                    </h3>

                    <div className="flex flex-col sm:flex-row items-center gap-4 w-full lg:max-w-2xl">
                        <div className="relative w-full">
                            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-muted opacity-40" size={16} />
                            <input
                                type="text"
                                placeholder="Buscar por beat o comprador..."
                                className="w-full bg-white/5 border border-white/10 rounded-2xl py-3 pl-12 pr-4 text-[11px] font-bold uppercase tracking-widest focus:outline-none focus:border-accent/40 transition-all"
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                            />
                        </div>
                        <select
                            className="bg-white/5 border border-white/10 rounded-2xl py-3 px-6 text-[10px] font-bold uppercase tracking-widest focus:outline-none focus:border-accent/40 transition-all appearance-none cursor-pointer"
                            value={filterLicense}
                ...
                <div className="space-y-4">
                    {filteredSales.map((sale) => (
                        <div key={sale.id} className="group flex flex-col md:flex-row md:items-center justify-between p-6 rounded-[2rem] bg-white/5 border border-white/5 hover:bg-white/10 hover:border-accent/20 transition-all duration-500">
                            <div className="flex items-center gap-6 flex-1">
                                {/* Beat Cover */}
                                <div className="relative w-16 h-16 rounded-2xl overflow-hidden border border-white/10 shadow-xl shrink-0 group-hover:scale-105 transition-transform duration-500">
                                ...
                                <div className="min-w-0">
                                    <div className="flex items-center gap-3 mb-2">
                                        <h4 className="font-black text-sm uppercase tracking-tight truncate max-w-[200px]">
                            ...
                            {/* Financial Details */}
                            <div className="flex items-center justify-between md:justify-end gap-12 mt-6 md:mt-0 pt-6 md:pt-0 border-t md:border-t-0 border-white/5">
                                ...
                                <div className="text-right min-w-[100px]">
                                    <p className="text-[9px] font-black text-emerald-500 uppercase tracking-[0.3em] mb-1">Impacto</p>
                                    <h5 className="font-black text-xl tracking-tighter text-foreground group-hover:text-emerald-500 transition-colors flex items-center justify-end gap-2">
                                        {sale.payment_method === 'paypal' ? (
                                            <span className="text-[#003087] font-serif italic font-black text-[10px] opacity-60">PP</span>
                                        ) : (
                                            <CreditCard size={14} className="text-muted opacity-20" />
                                        )}
                                        +{formatCurrency(sale.amount)}
                                    </h5>
                                </div>
                                <button className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center text-muted hover:bg-white/10 hover:text-white transition-all group-hover:border-white/20 border border-transparent">
                    ...
                    {filteredSales.length === 0 && (
                        <div className="py-24 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                            ...
                            <h4 className="text-lg font-black uppercase text-muted/40 tracking-tighter mb-2">No se encontraron transacciones</h4>
                ...
                {filteredSales.length > 10 && (
                    <div className="mt-12 flex justify-center">
                        <button className="bg-white/5 border border-white/10 px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all">
