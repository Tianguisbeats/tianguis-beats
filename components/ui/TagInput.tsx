...
export default function TagInput({ tags, setTags, placeholder = "Referencia (ej: Bad Bunny)", maxTags = 5 }: TagInputProps) {
    ...
    return (
        <div className="space-y-3">
            <div className="flex flex-wrap gap-2 min-h-[44px] p-2 bg-black/[0.03] dark:bg-white/[0.05] border border-border dark:border-white/20 rounded-2xl focus-within:border-blue-500/50 transition-all duration-300 shadow-sm">
                {tags.map((tag, index) => (
                    <span
                        key={index}
