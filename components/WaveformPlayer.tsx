...
export default function WaveformPlayer({
    ...
    useEffect(() => {
        ...
        // Sync time if difference is more than 0.5 seconds
        const wsTime = wavesurfer.current.getCurrentTime();
        if (Math.abs(wsTime - globalCurrentTime) > 0.5) {
            wavesurfer.current.setTime(globalCurrentTime);
        }
    }, [globalIsPlaying, globalCurrentTime, isActuallySyncing]);
