-- RESTAURACIÓN DE CONTADORES Y LOGICA FOUNDER (FIX v6)
-- Basado en "Musical Chairs" pero adaptado al esquema español actual (foto_perfil, fecha_de_creacion).

-- 1. ESTRUCTURA: Agregar columnas de contadores si no existen
...
-- Nota: Ya usábamos 'fecha_de_creacion' en el esquema v5, así que usamos esa.

-- 2. TRIGGER ASIGNACIÓN (Sillas Musicales)
...
BEGIN
  ...
  IF (TG_OP = 'INSERT') THEN
     ...
     -- 2. Asignar fecha_de_creacion (Fallback por si viene null)
     IF NEW.fecha_de_creacion IS NULL THEN
        NEW.fecha_de_creacion := now();
     END IF;

     -- 3. Asignación por Tier
  ...
  ELSIF (TG_OP = 'UPDATE' AND NEW.subscription_tier IS DISTINCT FROM OLD.subscription_tier) THEN
     -- Limpiar slots viejos y asignar nuevo
  ...
  -- 4. EVALUAR FOUNDER (Primeros 100 de Pro o Premium)
  -- Nota: Si el usuario ya era founder, no se lo quitamos automáticamente a menos que cambie de logica.
  -- Aquí aplicamos la regla estricta: Es founder si su número en Pro o Premium es <= 100.
  NEW.is_founder := (
...
-- Borrar trigger anterior si existía para evitar duplicados
...
-- 3. TRIGGER RE-SECUENCIA (Cerrar huecos al borrar/cambiar)
...
BEGIN
  ...
  -- Detectar hueco por cambio de tier o borrado
  IF (TG_OP = 'DELETE') OR (TG_OP = 'UPDATE' AND OLD.subscription_tier IS DISTINCT FROM NEW.subscription_tier) THEN
     ...
     IF v_old_tier = 'pro' AND v_old_pro IS NOT NULL THEN
        ...
        -- Si al recorrer, alguien cae en el puesto 100, se vuelve founder
...
-- 4. RECALCULO INICIAL (Por si ya hay datos)
DO $$
DECLARE
  r RECORD;
  c_total INT := 0;
  ...
  c_prem INT := 0;
BEGIN
  -- Usamos 'fecha_de_creacion' que es la columna oficial actual
  FOR r IN SELECT id, subscription_tier FROM public.profiles ORDER BY fecha_de_creacion ASC, id ASC
  LOOP
    c_total := c_total + 1;
  ...
  -- Actualizar founders tras el recálculo
...
SELECT 'Triggers Musical Chairs Restaurados y Optimizados para Esquema v5' as status;
