export type LicenseType = 'basic' | 'premium' | 'unlimited' | 'exclusive' | 'soundkit' | 'service' | 'gratis';
...
export const generateLicenseText = (type: LicenseType, data: LicenseData): string => {
    ...
    switch (type) {
        ...
        case 'premium':
            return header + commonIntro +
                `1. TIPO DE LICENCIA: LICENCIA PRO (WAV)\n` +
                `En consideración del pago de la Tarifa, el Productor otorga al Licenciatario el derecho no exclusivo, intransferible y mundial para usar el Beat en la creación de una (1) Nueva Canción final.\n\n` +
                `2. FORMATO DE ENTREGA\n` +
                `El Productor entregará un archivo de alta calidad en formato WAV integral.\n\n` +
                `3. PERMISOS Y LÍMITES DE EXPLOTACIÓN\n` +
        ...
        case 'unlimited':
            return header + commonIntro +
                ...
                `2. FORMATO DE ENTREGA\n` +
                `El Productor entregará WAV y "Trackouts" (Stems Separados) del Beat.\n\n` +
        ...
        case 'service':
            return header +
                `Este comprobante certifica la contratación del Servicio Profesional: "${productName}" proporcionado por ${producerName}.\n\n` +
                `1. ALCANCE DEL SERVICIO\n` +
                `El Productor se compromete a entregar el trabajo solicitado (Mezcla, Masterización o Producción Personalizada) siguiendo los estándares de calidad discutidos. El tiempo de entrega estimado y los detalles técnicos están sujetos a lo especificado en la plataforma Tianguis Beats.\n\n` +
                `2. PROPIEDAD INTELECTUAL\n` +
                `Una vez completado el pago y entregado el material final, los derechos de explotación sobre el trabajo resultante se transferirán al Cliente, a menos que se acuerde lo contrario (como en el caso de regalías por composición si el productor participa en la creación melódica).\n\n` +
                `3. COMUNICACIÓN Y REVISIONES\n` +
                `El Cliente deberá proporcionar el material necesario para iniciar el trabajo. Las revisiones incluidas se gestionarán a través del chat o panel de proyecto en el Studio de Tianguis Beats.\n\n` +
                `4. IDENTIFICADOR DE TRANSACCIÓN\n` +
                `Este documento sirve como comprobante de fe de la transacción realizada bajo el identificador ${orderId}.\n`;
